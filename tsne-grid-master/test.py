...
parser.add_argument('-s', '--size', type=int, help="number of small images in a row/column in the output image")
parser.add_argument('-d', '--dir', type=str, help="source directory for images")
parser.add_argument('-r', '--res', type=int, default=224, help="width/height of the output square image")
parser.add_argument('-n', '--name', type=str, default='tsne_grid.jpg', help='name of the output image file')
parser.add_argument('-p', '--path', type=str, default='./', help="destination directory for the output image")
parser.add_argument('-x', '--per', type=int, default=50, help="TSNE perplexity")
parser.add_argument('-i', '--iter', type=int, default=5000, help="number of iterations in the TSNE algorithm")
...
if os.path.exists(args.dir):
    ...
    raise argparse.ArgumentTypeError("'{}' not a valid directory.".format(args.dir))

if os.path.exists(args.path):
    out_dir = args.path
else:
    raise argparse.ArgumentTypeError("'{}' not a valid directory.".format(args.path))
...
def load_img(in_dir):
    ...
    for idx, img in enumerate(pred_img):
        ...
        img_collection.append(Image.open(img).resize((out_res, out_res)))

    if (np.square(out_dim) > len(img_collection)):
        raise ValueError("Cannot fit {} images in a {}x{} grid".format(len(img_collection), out_dim, out_dim))
...
def get_activations(model, img_collection):
    ...
    for idx, img in enumerate(img_collection):
        ...
        activations.append(np.squeeze(model.predict(img)))
...
def save_tsne_grid(img_collection, X_2d, out_res, out_dim):
    ...
    #cost_matrix = scipy_distance.cdist(grid, X_2d, "sqeuclidean").astype(np.float32)
    ...
    for pos, img in zip(grid_jv, img_collection[0:to_plot]):
        ...
        out[h_range:h_range + out_res, w_range:w_range + out_res] = np.array(img)



    im = Image.fromarray(np.uint8(out))
    im.save(os.path.join(out_dir, out_name), quality=100)
